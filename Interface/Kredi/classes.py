...
def get_loan_info(cus_id):
    ...
    if loan_row:
        ...
        for i in range(0, int(loan_row.ay)):
            ...
            # anapara column
            anapara = float(loan_row.anapara) / float(loan_row.ay)
            arr.append(anapara)
            # faiz column
            faiz = (float(loan_row.anapara)*int(loan_row.faiz)) / \
                (float(loan_row.ay)*100)
            arr.append(faiz)
            # gecekme column
            gecikme = 0
            arr.append(gecikme)
...
# date_after_month = dt_obj + relativedelta(months=1)
# print('Today: ', dt_obj.strftime('%Y-%m-%d'))
# print('After Month:', date_after_month.strftime('%Y-%m-%d'))
